...
async function runtimeSummary(i: ChatInputCommandInteraction): Promise<string> {
...
staticCommand({
  ...
  category: 'information',
  content:
    'The official JerSuit support community is the best place to report bugs, request features and talk with maintainers. Ask a maintainer in the dashboard for the current link.',
});
...
staticCommand({
  ...
  category: 'information',
  content:
    'Thanks for considering a vote! Voting helps keep JerSuit free and pushes it up the listing so more communities can discover it. Voting will open from the dashboard soon. 💚',
});
...
staticCommand({
  ...
  category: 'information',
  content:
    'Documentation, setup guides and FAQs live inside the dashboard. Open **Commands** to browse every command and visit a server dashboard to configure each feature in real time.',
});
...
staticCommand({
  ...
  category: 'information',
  content:
    'JerSuit is a modular Discord management platform. The dashboard gives you the full experience — landing page, server list, guild configuration, the command library, and more.',
});
...
staticCommand({
  ...
  guildOnly: true,
  content:
    'JerSuit is slash-first: type a forward slash **/** in any channel and pick a command from the list. Every command shows its options inline.',
});
...
staticCommand({
  ...
  guildOnly: false,
  content:
    'By using JerSuit you agree to follow Discord’s Terms of Service and Community Guidelines. JerSuit may not be used for harassment, spam, raids, or illegal activity. Server owners are responsible for how they configure the bot in their communities.',
});
...
staticCommand({
  ...
  guildOnly: false,
  content:
    'JerSuit stores only what is required to operate: guild settings, moderation cases, economy/levels data (when enabled) and dashboard sessions. Your Discord token is never exposed or published. You can request data deletion at any time via a maintainer.',
});
...
staticCommand({
  ...
  builder: (b) =>
    b.addChannelOption((o) => o.setName('channel').setDescription('Channel to inspect').setRequired(false)),
});