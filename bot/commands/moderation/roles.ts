...
defineCommand({
  ...
  execute: async (ctx) => {
    ...
    if (!guild || !('id' in role)) return error(ctx.interaction, 'Invalid role.');
    if (role.id === guild.roles.everyone.id) return error(ctx.interaction, 'Cannot assign @everyone.');
    const member = await guild.members.fetch(user.id).catch(() => null);
    if (!member) return error(ctx.interaction, 'That user is not a member of this server.');
    if (member.roles.cache.has(role.id)) return error(ctx.interaction, 'That member already has this role.');
...
defineCommand({
  ...
  execute: async (ctx) => {
    ...
    const member = await guild.members.fetch(user.id).catch(() => null);
    if (!member) return error(ctx.interaction, 'That user is not a member of this server.');
    if (!member.roles.cache.has(role.id)) return error(ctx.interaction, 'That member does not have this role.');
...
defineCommand({
  ...
  execute: async (ctx) => {
    ...
    if (!guild) {
      return error(ctx.interaction, 'This command only works in servers.');
    }
...
defineCommand({
  ...
  execute: async (ctx) => {
    ...
    if (!role) {
      return error(ctx.interaction, 'That role could not be found.');
    }

    const name = role.name;

    if (!role.editable) {
      return error(ctx.interaction, 'That role cannot be deleted by the bot.');
    }
...
defineCommand({
  ...
  execute: async (ctx) => {
    ...
    const color = ctx.interaction.options.getString('color') ?? '#33A65F';
    await role.setColor(color as `#${string}`);
...
defineCommand({
  ...
  execute: async (ctx) => {
    const role = ctx.interaction.options.getRole('role', true);
    if (!('members' in role)) return error(ctx.interaction, 'Role not found.');
    let count = 0;
