...
function toFixed2(x: number): string { return Number(x.toFixed(4)).toString(); }
function fact(n: number): number { let r = 1; for (let k = 2; k <= n; k++) r *= k; return r; }
function gcd2(a: number, b: number): number { a = Math.abs(a); b = Math.abs(b); while (b) { const t = b; b = a % b; a = t; } return a; }
function lcm2(a: number, b: number): number { return Math.abs(a * b) / gcd2(a, b); }
...
function toRoman(n: number): string { if (n < 1 || n > 3999) return 'out of range'; const v: [number, string][] = [[1000,'M'],[900,'CM'],[500,'D'],[400,'CD'],[100,'C'],[90,'XC'],[50,'L'],[40,'XL'],[10,'X'],[9,'IX'],[5,'V'],[4,'IV'],[1,'I']]; let out = ''; let x = Math.trunc(n); for (const [val, sym] of v) { while (x >= val) { out += sym; x -= val; } } return out; }
function fromRoman(s: string): number { const m: Record<string, number> = { I: 1, V: 5, X: 10, L: 50, C: 100, D: 500, M: 1000 }; let t = 0; for (let k = 0; k < s.length; k++) { const c = m[s[k]] ?? 0; const nx = m[s[k + 1]] ?? 0; t += c < nx ? -c : c; } return t; }
function shaH(algo: string, s: string): string { const { createHash } = require('node:crypto'); return createHash(algo).update(s).digest('hex'); }
