...
exports.generateToken = exports.getDbHealth = exports.getDb = void 0;
...
`;
async function seedIfEmpty(db, table, sql) {
...
async function openPGlite(dataDir) {
    ...
    try {
        ...
        const probe = await db.query('SELECT 1 AS v');
        void probe;
        return db;
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        logger_1.logger.error('db', 'PGlite failed to open data directory - attempting recovery', {
            error: message,
            dataDir,
        });
        ...
        try {
            ...
            logger_1.logger.error('db', 'PGlite recovery: could not move corrupt data directory', {
                error: renameMessage,
            });
            throw err;
        }
        logger_1.logger.warn('db', 'PGlite recovered: initializing a fresh data directory', {
            backup: corruptDir,
        });
        return new pglite_1.PGlite(dataDir);
    }
}
async function initDb() {
    ...
    const dataDir = process.env.PGLITE_DATA_DIR || `${process.cwd()}/.data/jersuit-v2`;
    const db = await openPGlite(dataDir);
    await db.exec(SCHEMA_SQL);
    await db.exec(MIGRATIONS_SQL);
    // Seed default rows if empty
    await seedIfEmpty(db, 'bot_configuration', "INSERT INTO bot_configuration (status, activity_type, activity_name, token_configured) VALUES ('offline', 'playing', NULL, false)");
    await seedIfEmpty(db, 'bot_status', "INSERT INTO bot_status (state) VALUES ('offline')");
    await seedIfEmpty(db, 'appearance_settings', 'INSERT INTO appearance_settings DEFAULT VALUES');
    // Seed platform_settings defaults (key-value)
    for (const [key, value] of Object.entries(DEFAULT_PLATFORM_SETTINGS)) {
        await db.query(`INSERT INTO platform_settings (key, value)
       SELECT $1, $2::jsonb
       WHERE NOT EXISTS (SELECT 1 FROM platform_settings WHERE key = $1)`, [key, JSON.stringify(value)]);
    }
    return db;
}
...
async function getDbHealth() {
    try {
        ...
        await db.query('SELECT count(*) AS cnt FROM users');
