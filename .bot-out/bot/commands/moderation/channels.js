...
const MANAGE = discord_js_1.PermissionFlagsBits.ManageMessages;
...
(0, framework_1.defineCommand)({
    ...
    cooldown: 4,
    usage: '/clear <amount>',
    builder: (b) => b.addIntegerOption((o) => o.setName('amount').setDescription('Messages to delete (max 100)').setRequired(true).setMinValue(1).setMaxValue(100)),
    execute: async (ctx) => {
        const amount = ctx.interaction.options.getInteger('amount') ?? 1;
        const channel = ctx.interaction.channel;
        if (!channel || !('bulkDelete' in channel))
            return (0, framework_1.error)(ctx.interaction, 'This command only works in text channels.');
        const deleted = await channel.bulkDelete(Math.min(amount, 100), true).catch(() => null);
        if (deleted === null)
            return (0, framework_1.error)(ctx.interaction, 'Could not delete messages (too old or missing permissions).');
...
(0, framework_1.defineCommand)({
    ...
    memberPermissions: MANAGE,
    cooldown: 3,
    builder: (b) => b.addIntegerOption((o) => o.setName('amount').setDescription('Messages to delete').setRequired(true).setMinValue(1).setMaxValue(100)),
    execute: async (ctx) => {
        ...
        const deleted = await channel.bulkDelete(Math.min(amount, 100), true).catch(() => null);
        if (deleted === null)
            return (0, framework_1.error)(ctx.interaction, 'Could not delete messages.');
...
(0, framework_1.defineCommand)({
    ...
    memberPermissions: MANAGE,
    cooldown: 3,
    builder: (b) => b.addIntegerOption((o) => o.setName('seconds').setDescription('Slowmode in seconds (0 disables)').setRequired(true).setMinValue(0).setMaxValue(21600)),
    execute: async (ctx) => {
        ...
        await ctx.interaction.reply({
            embeds: [
                modEmbed('Slowmode', seconds > 0 ? `Slowmode set to **${seconds}s** in ${channel}.` : 'Slowmode **disabled** in this channel.'),
            ],
        });
    },
});
(0, framework_1.defineCommand)({
    ...
    execute: async (ctx) => {
        ...
        if (!guild || !channel || !('permissionOverwrites' in channel)) {
            return (0, framework_1.error)(ctx.interaction, 'This command only works in server text channels.');
        }
        await channel.permissionOverwrites.edit(guild.roles.everyone, { SendMessages: false }, { reason: 'lock command' });
...
(0, framework_1.defineCommand)({
    ...
    execute: async (ctx) => {
        ...
        if (!channel || !('clone' in channel))
            return (0, framework_1.error)(ctx.interaction, 'This channel cannot be nuked.');
        ...
        if (clone)
            await clone.setPosition(position).catch(() => undefined);
...
(0, framework_1.defineCommand)({
    ...
    execute: async (ctx) => {
        const channel = ctx.interaction.channel;
        if (!channel || !('clone' in channel))
            return (0, framework_1.error)(ctx.interaction, 'This channel cannot be cloned.');
