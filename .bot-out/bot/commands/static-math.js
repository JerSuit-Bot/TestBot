...
const num = (i, k) => i.options.getNumber(k) ?? 0;
const int = (i, k) => i.options.getInteger(k) ?? 0;
function fact(n) {
    let r = 1;
    for (let k = 2; k <= n; k++)
        r *= k;
...
(0, batch_1.batch)([
    ...
    },
    {
        ...
        run: (i) => {
            const n = int(i, 'n');
            if (n < 0 || n > 170)
                return 'Provide an integer from 0 to 170.';
    ...
    },
    {
        ...
        run: (i) => `**Interest:** ${(num(i, 'principal') * (num(i, 'rate') / 100) * num(i, 'years')).toFixed(2)} **Total:** ${(num(i, 'principal') * (1 + (num(i, 'rate') / 100) * num(i, 'years'))).toFixed(2)}`,
    },
    {
        name: 'dice',
