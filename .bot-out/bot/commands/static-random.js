...
function sh256(s) { const { createHash } = require('node:crypto'); return createHash('sha256').update(s).digest('hex'); }
...
(0, batch_1.batch)([
    ...
    },
    {
        ...
        category: 'utility',
        run: () => { const len = Math.min(Math.max(gi(i, 'length'), 1), 64); const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'; let out = ''; for (let k = 0; k < len; k++)
            out += chars[Math.floor(Math.random() * chars.length)]; return '**' + out + '**'; },
    },
    {
        name: 'random-roll',
