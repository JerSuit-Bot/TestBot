...
function binaryToText(s) {
    ...
    for (const p of parts) {
        if (!/^[01]{1,8}$/.test(p))
            return 'Invalid binary input (8-bit chunks).';
...
const reverseStr = (s) => Array.from(s).reverse().join('');
...
const toCamel = (s) => s
    ...
    .join('');
const toSnake = (s) => s.split(/[^a-zA-Z0-9]+/).filter(Boolean).map((w) => w.toLowerCase()).join('_');
const toKebab = (s) => s.split(/[^a-zA-Z0-9]+/).filter(Boolean).map((w) => w.toLowerCase()).join('-');
const toPascal = (s) => s
    .split(/[^a-zA-Z0-9]+/)
    .filter(Boolean)
...
(0, batch_1.batch)([
    ...
    },
    {
        ...
        category: 'utility',
        run: () => `\`${(0, node_crypto_1.randomUUID)()}\``,
    },
    {
        name: 'palindrome',
    ...
    },
    {
        ...
        run: (i) => {
            ...
            let out = '';
            for (let k = 0; k < len; k++)
                out += chars[bytes[k] % chars.length];
            return `\`${out}\``;
        },
    },
    {
    ...
    },
    {
        ...
        opts: [{ name: 'text', description: 'Text', required: true, type: 'string' }],
        run: (i) => {
            const m = { a: '4', e: '3', i: '1', o: '0', s: '5', t: '7', g: '9' };
...
function createHashOf(algo) {
    return require('node:crypto').createHash(algo);
}
